...
function tournamentView() {
    ...
    let html = "";
    let players = model.inputs.newTournament.participants.map(user => `<div style="display: flex; justify-content: space-between; padding: 1rem"><span>${user}</span><button onclick="deletePlayer2(${user})">Hei</button></div>`)
    html += `
...
function listUsers() {
    ...
    for (let i = 0; i < users.length; i++) {
        if (model.inputs.newTournament.participants.includes(users[i].userName)) { continue }
