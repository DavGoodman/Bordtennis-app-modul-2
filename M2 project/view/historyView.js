...
function historyView() {
  ...
  const recentMatches = listAllEvents(currentUser)
    ...
    <tr onclick="getEvent(${index})">
      <td><span class="history-vs">vs </span>${getOpponent(item)}</td>
  ...
  isEmpty(currentEvent) === true
            ...
            recentMatches.length == 0
              ...
              <table>
                <thead>
                  ...
                  <i onclick="model.inputs.history.showAll = !model.inputs.history.showAll; historyView();" class="fa-solid fa-ellipsis expand"></i>
                  </div>
              `
          }
    ...
    <div class="logout-div" onclick="model.inputs.history.showEvent = {}; historyView();">  
    <i class="fa-solid fa-arrow-right-from-bracket log-out-btn"></i>
    </div>
    <div class="logo-container">
    <svg width="155" height="97" viewBox="0 0 155 97" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <div class="history-selected">
      <div style="display: flex; justify-content: space-between;">Dato: <span>${convertTime(
        currentEvent.datePlayed
      )}</span></div>
      <div style="display: flex; justify-content: space-between;">Kl: <span>${convertTimeSpecific(
        currentEvent.datePlayed
      )}</span></div>
      <div style="display: flex; justify-content: space-between;"><span ${
        currentEvent.participants[0].matchScore === 10 ? `style="color:#ee7100;"` : ""
      }>${getUserName(currentEvent.participants[0].playerId)}:</span> <div>${
        currentEvent.participants[0].matchScore
      }</div></div>
      <div style="display: flex; justify-content: space-between;"><span ${
        currentEvent.participants[1].matchScore === 10 ? `style="color:#ee7100;"` : ""
      }>${getUserName(currentEvent.participants[1].playerId)}:</span> <div>${
    ...
    <div class="history-selected">
    <div style="display: flex; justify-content: space-between;">Tittel:   <span>${
      currentEvent.tournamentName
    }</span></div>
    <div style="display: flex; justify-content: space-between;">Dato:   <span>${convertTime(
      currentEvent.datePlayed
    )}</span></div>
    ...
    <div>Deltakere:   
    <span>${getTournamentPlayers()
      .map(
        (player) =>
          `<div ${player === getUserName(currentEvent.winnerId) ? `style="color:#ee7100;"` : ""}>${player}</div>`
      )
      .join("")}
      </span>
      </div>
...
function convertTimeSpecific(t) {
  d = new Date(t);
